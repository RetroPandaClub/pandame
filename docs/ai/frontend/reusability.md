...
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Avatar`                | Round image with text-initials fallback. Sizes sm 32 / md 48 / lg 76 / xl 134 px (matching Figma usage). Pass `src` (e.g. `profile.avatar_url`, which `ensureProfile` defaults to a deterministic DiceBear URL — see `$lib/utils/avatar.utils.ts`).                                                                                                                                                              |
...
| `BrandHeaderBackButton` | White-on-tint chevron-left for any `BrandHeader.leading` slot. 44 × 44 hit area with a 24 × 24 `BackIcon`. `ariaLabel` defaults to `core.text.back_to_dashboard`; pass it explicitly for context-specific labels (e.g. dispute "Cancel"). Use this anywhere a coloured header needs a back affordance — don't reach for `IconButton variant="ghost"` (its `text-default` resolves to dark on tinted headers).    |
...
| `PanelSizePicker`       | Three concentric-circle radios picking the per-deal arbitrator count (Figma `Fast = 3` / `Fair = 7 (Recommended)` / `Less fast = 11`). Bound `value: number`. Threads into `CreateDealArgs.panel_size`; the canister rejects out-of-range values with `PanelSizeOutOfRange { min, max, got }`. Constants live in `$lib/constants/dispute.constants.ts` (`PANEL_SIZE_FAST` / `_FAIR` / `_SLOW` / `_DEFAULT = 7`). |
| `VotePicker`            | Stacked CC / IC / Abstain radio cards with title + description per option. `excludeAbstain` hides Abstain (the canister rejects it for `withdraw_dispute` proposals). Used by the dispute page (panel cast-vote + party withdraw flows).                                                                                                                                                                         |
| `WalletTokenList`       | One `Money`-formatted balance row per token. Accepts a `tokens` prop (defaults to `SUPPORTED_TOKENS`); fetches each balance lazily via `myBalance` and surfaces per-row `loading` / `error` states so an unreachable ledger doesn't blank the whole list. Used by `/profile/wallet`.                                                                                                                             |
| `DisputePhaseBadge`     | 24 px pill rendering the dispute phase (Evidence / Voting / Resolved) with phase-tinted background. Mirrors `DealStatusIcon` but for disputes.                                                                                                                                                                                                                                                                   |
| `EvidenceList`          | List view for `Evidence[]` — submitter, timestamp, free-form note, off-canister artefact URL + SHA-256 hash (rendered as monospace hex). Empty-state when the dispute has no evidence yet.                                                                                                                                                                                                                       |
| `EvidenceForm`          | Three-field form (note / URL / hash) for `submit_evidence`. Bindable; the parent calls `submit_evidence` and resets the bound fields on success.                                                                                                                                                                                                                                                                 |
| `DisputeCard`           | Single-dispute preview (red title bar with `Deal #N` + `DisputePhaseBadge` + arbitration-fee + countdown to the live deadline). Used by `/transactions` (Disputed tab) and `/profile/arbitrator`.                                                                                                                                                                                                                |
...
| `Auth`               | Behaviour-only: restores the session via `initAuth` and hydrates `userStore`, bootstraps `profileStore` via `ensureProfile` (resets it on sign-out) so trailing-slot widgets like `UserPrincipalBadge` always have an avatar, Mounted once at the layout level — no UI.                                                                                                                                             |
| `AvatarUploadSheet`  | Modal with up to three actions — Take photo (front camera), Choose from library, and (optional) Use generated avatar — backed by hidden `<input type="file" accept="image/*">`s. Parent receives the `File` via `onpicked` and handles compression + persistence; the optional `onpickeddefault` callback lets the parent reset to the principal-derived DiceBear default.                                          |
...
| `DealActions`        | Context-aware action bar. On `Created`: Consent / Reject / Cancel. On `Funded` for a bound deal: Confirm completion (`signYes`) + Reject completion (`signNo`) for whichever party still carries an `Empty` signature. On `Funded` for a tip flow: Accept (recipient claims via `accept_deal`). Payer also gets Reclaim once the deal is expired. A "Dispute (soon)" stub navigates to `/deals/[id]/dispute`.       |
...
| `DealsLoader`        | Behaviour-only: while signed in, fires `pandameReloadDeals` immediately and on a 30 s `setInterval`; the same window listener catches both timer ticks and the `emit` calls from action services and pages, then refreshes `dealsStore` + `disputesStore`. Resets both stores on sign-out. Mounted once in `+layout.svelte` next to `<Auth />`. Mirrors oisy-wallet's `LoaderTokens` / `LoaderUserProfile` pattern. |
...
| `UserPrincipalBadge` | `BrandHeader` `trailing` slot for authenticated routes — caller's short principal + Avatar. The Avatar mirrors `profileStore.data.avatar_url` (bootstrapped centrally by `Auth.svelte` after sign-in) so the header icon always matches the big Profile avatar. Click copies the full `userPrincipalText` to the clipboard with a brief inline check + "Copied!" swap (~1.5 s).                                     |
...
| `deal.services`     | Deal lifecycle orchestration (`createAndFundDeal`, `acceptDeal`, `consentDeal`, `signYes`, `signNo`, `rejectDeal`, `cancelDeal`, `reclaimDeal`, `listMyDeals`, `getDeal`, `getClaimableDeal`, `getReliability`). Action wrappers upsert the returned `DealView` into `dealsStore` and emit `pandameReloadDeals` so the badge / open lists refresh without per-callsite plumbing. Also exports the typed five-role `preApprovalAmount(call)` calculator + `DEFAULT_CREATION_FEE` constant used by `/deals/new` to surface the anti-spam fee line. |
| `dispute.services`  | Dispute lifecycle orchestration (`openDispute`, `submitEvidence`, `castVote`, `finalizeDispute`, `withdrawDispute`, `getDispute`, `getPublicDispute`, `listMyDisputes`). Action wrappers upsert into `disputesStore` and emit `pandameReloadDeals` (same pattern as `deal.services`).                                                                                                                                                                                                                                                            |
| `balance.services`  | `myBalance({ token? })` — caller's principal default subaccount on the chosen ledger (defaults to ICP).                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `profile.services`  | Editable `UserProfile` persistence in the `profiles` datastore (`getProfile`, `upsertProfile`, `ensureProfile`). `ensureProfile` also seeds / backfills `avatar_url` with the principal-derived DiceBear default.                                                                                                                                                                                                                                                                                                                                |
| `treasury.services` | Controller-only escrow treasury readers / drainers (`treasuryBalance`, `treasuryWithdraw`). Wraps `admin_treasury_balance` + `admin_treasury_withdraw`; non-controllers see `NotAuthorised`. Used by the `/profile/admin` Treasury panel.                                                                                                                                                                                                                                                                                                        |
...
| `deals.derived`   | `$lib/derived/` | `dealsLoaded`, `dealsCount`, `activeDeals`, `settledDeals`, `refundedDeals`, `cancelledDeals`, `dealsAttentionCount` (open deals waiting on the signed-in user — drives the nav badge). |
...
| `deal.utils`         | `dealStatus`, `consentState`, `signatureState`, `isTerminal`, `isExpired`, `sideOf`, `isFullyConsented`.                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `avatar.utils`       | `defaultAvatarUrlForPrincipal` / `dicebearAvatarUrl` — deterministic DiceBear (`notionists`) URL derived from an FNV-1a 64-bit hash of the principal alone, so the raw principal never leaves the client. Used by `ensureProfile` to seed `profile.avatar_url` on first sign-in.                                                                                                                                                                                                                                      |
| `escrow-error.utils` | `friendlyEscrowError(err, $i18n, token?)` — maps `EscrowCanisterError.variant` to a translated string via the `i18n.errors.*` namespace. Falls back to `err.message` for unmapped variants. Use this wherever an `escrowApi.*` call can throw.                                                                                                                                                                                                                                                                        |
| `events.utils`       | `emit({ message, detail? })` — tiny `document.dispatchEvent` wrapper for the project's typed custom events (declared in `src/custom-events.d.ts`). Pair with `<svelte:window on{Message}={handler} />` to react. Today: `DealsLoader` owns the lone `pandameReloadDeals` listener that refreshes deals + disputes; the action services, a 30 s timer in `DealsLoader`, and pages on mount all `emit` the same event. Project-local events use the `pandame*` prefix; library-emitted ones keep their original prefix. |

### Types — `$lib/types/`

| Type        | Purpose                                                                                                                  |
| ----------- | ------------------------------------------------------------------------------------------------------------------------ |
| `canister`  | `CanisterIdText` (zod via `@dfinity/zod-schemas`) + `CreateCanisterOptions<T>`.                                          |
| `deal`      | Re-exports `Deal` / `ClaimableDeal` / `DealError` / `DealStatusKey` / `ConsentKey` / `SignatureKey`; defines `DealSide`. |
| `profile`   | `UserProfile` interface + `emptyProfile` factory (shape stored in the `profiles` collection).                            |
| `token`     | App-side `Token` shape.                                                                                                  |
| `user`      | The signed-in `User` (principal text), resolved from the delegation.                                                     |
