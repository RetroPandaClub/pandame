...
| `Auth`               | Behaviour-only: subscribes to `onAuthStateChange` and hydrates `userStore`, bootstraps `profileStore` via `ensureProfile` (resets it on sign-out) so trailing-slot widgets like `UserPrincipalBadge` always have an avatar, and listens for `junoSignOutAuthTimer`. Mounted once at the layout level — no UI.                                                                                                       |
...
| `profile.services`  | Editable `UserProfile` persistence in the Juno `profiles` datastore (`getProfile`, `upsertProfile`, `ensureProfile`). `ensureProfile` also seeds / backfills `avatar_url` with the principal-derived DiceBear default.                                                                                                                                                                                                                                                                                                                           |
