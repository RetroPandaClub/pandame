...
│   ├── index.ts
│   ├── escrow/
│   └── satellite/          Generated by `npm run juno:functions:build` once
│                           an extension query/update lands (empty today)
│
├── satellite/              Juno serverless functions (the satellite WASM extension)
│   ├── index.ts            `defineHook` / `defineAssert` / `defineQuery` / `defineUpdate` exports
│   └── tsconfig.json       Isolated TS config — bundled by `juno functions build`
│
...
- Hand-edits to anything under `src/declarations/escrow/` — generated
  by `npm run did`.
- Hand-edits to anything under `src/declarations/satellite/` — generated
  by `npm run juno:functions:build` (Juno CLI). The folder only
  appears once the satellite exposes at least one `defineQuery` /
  `defineUpdate` export.
